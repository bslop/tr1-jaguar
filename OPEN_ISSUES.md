...
| A10 | **A build can black-screen silicon from t=0 with no crash and no emulator symptom** | **OPEN — the top blocker; 12 flashes spent 2026-07-29** | This is the "jagemu passes, silicon dies" class (now 8+ cases) given a name. **The moveset was NOT the cause** and neither were the six earlier suspects. Measured, one flash each, outcome read off the capture: no-moveset control **boots**; +1024 B dead `.rodata` (size-matched to the failing ROM) **boots**; +1016 B dead `.text` (so `.rodata`/`.data`/`.bss` land at the failing ROM's exact addresses) **boots**; roll only, +912 B of real code, **boots**; sidestep only, +560 B, **BLACK**; sidestep + 4 volatile stores (`BOOTCRUMBS`) **boots**; `gd_install` stubbed out **BLACK** (GD BIOS exonerated); `SYNCPOLL=1`, which removes the `STOP` from `gpu_sync` entirely, **BLACK** (that STOP exonerated) — both of those with `main.o` byte-identical to the failing build, so they isolate one mechanism each. ☠️ **And the control source built with nothing but `-frename-registers` — a semantics-preserving register shuffle, zero source change — is BLACK.** So a large enough register reallocation in `main()` is sufficient on its own. **Ruled out:** image size, absolute addresses, phrase alignment of the framebuffers/OP list (checked every symbol: only 4-byte scalars change class), a 68k exception (the red crash beacon never paints), the GD BIOS, and `gpu_sync`'s STOP. ⚠️ **RETRACTED — my colour-beacon instrument was never validated and is BLIND.** `HANGDIAG` bounded every unbounded wait and painted BG (`$F00058`) on timeout; a milestone trail painted BG at each boot milestone. Both read black — but `O_TRAIL` demonstrably executed its markers and *booted*, and its border was black too, so **BG is not visible in the capture** (the OP object covers the active area; the pillarbox is blanking, not BG). The three conclusions that rested on it — "no wait timed out", "the vblank ISR never runs", "it never reaches `jsr main`" — are **withdrawn**. Any future beacon must paint into the FRAMEBUFFERS (`beacon_paint` already does, and is C-callable) and must be validated on a build that is known to boot BEFORE it is trusted. **Determinism:** per build, not per boot — `D_SIDE` was flashed twice and was black both times. **Workaround that ships:** moving the sidestep body into a `noinline` helper drops main()'s changed-instruction count from 522 to 337 and **boots** (`S_SHIP2`, verified). That is a mitigation, not a fix: the next feature re-rolls the same lottery, and the whole polished-demo plan is more edits to this one 6,000-instruction function. |
