...
/* ☠☠ HRESN: this used to be (FOCAL * RENDER_H / 240), which silently assumes
   the render is full width.  FOCAL scales with VIEW_W, so without the
   RENDER_W/VIEW_W term a half-width build halves FOCAL_Y too and DOUBLES the
   vertical FOV - the world renders half as tall and nothing errors.
   The invariant is FOCAL*hscale == FOCAL_Y*vscale, with hscale = RENDER_W/VIEW_W
   and vscale = 240/RENDER_H.  Reduces to the old expression when VIEW_W==RENDER_W,
   so the 320-wide builds stay bit-identical. */
#define FOCAL_Y   (FOCAL * RENDER_W / VIEW_W * RENDER_H / 240)
#define NEAR      32
...
#define PCL_FOCAL   (190 * VIEW_W / RENDER_W)  /* MUST match the kernel's FOCAL */
#define PCL_CX      (VIEW_W / 2)
#define PCL_CY      (VIEW_H/2)
#define PCL_FOCALY  (PCL_FOCAL * RENDER_W / VIEW_W * (RENDER_H) / 240)   /* was ==240?190:95 - which
...
static int portal_rect(const long *pr, int *rx0, int *rx1, int *ry0, int *ry1)
{
    ...
    *ry0 = miny - 1; *ry1 = maxy + 1;
    if (*rx0 < 0) *rx0 = 0;  if (*rx1 > (VIEW_W-1)) *rx1 = (VIEW_W-1);
...
#ifdef LEVSD
#include "lev_arena.h"        /* GENERATED by build_cof.sh from the packed
                                 images - never type this size by hand. */
/* ── BOTH LEVEL SETS LIVE ON THE CARD, SHARING ONE ARENA ──────────────────
   Sized for the LARGER image, because only one set is ever live. That is the
   whole point: the mansion alone would be net-zero (delete 254 KB of rodata,
   need a 254 KB buffer), and the saving only appears when the Caves are on the
   card too and the two share this. Frees 254,400 B of DRAM and takes ~1 MB out
   of the ROM.
   ☠️ 32-byte aligned. The file packs every blob 32-aligned so a load into a
   32-aligned arena preserves it; 16-mod-32 is the alignment class behind the
   A10 blackouts, and the atlas and geometry are read by the Blitter and Tom. */
static uint8_t g_lev_arena[LEV_ARENA_BYTES] __attribute__((aligned(32)));
static int g_lev_in = -1;              /* which set is resident: 0 caves 1 gym */

/* 0 on success. Negative codes are distinct on purpose - "it did not load" and
   "it loaded something that is not a level" want different answers, and a
   silent failure here renders garbage rather than reporting anything. */
static int lev_load(int set)
{
    ...
    int h = -1, i, sz;
    unsigned got = 0, need;
    if (g_lev_in == set) return 0;                 /* already resident */
    for (i = 0; i < 2 && h < 0; i++)
        h = gd_fopen(nm[set][i], GD_FOPEN_READ | GD_FOPEN_OPEN_EXISTING);
    if (h < 0) return -1;                          /* no card, or no file */
    sz = gd_fsize((unsigned)h);
    if (sz <= 0 || (unsigned)sz > (unsigned)LEV_ARENA_BYTES) {
        gd_fclose((unsigned)h); return -2;         /* refuses to overrun */
    }
    need = (unsigned)sz;
    while (got < need) {
        unsigned n = need - got;
        if (n > 16384u) n = 16384u;    /* ☠️ GD BIOS: <=24KB per call, and a big
                                          single read stalls the 68k for
                                          hundreds of ms - the music path
                                          learned that as pad lag. */
        if (gd_fread((unsigned)h, g_lev_arena + got, n, GD_FREAD_CPU) != 0) {
            gd_fclose((unsigned)h); g_lev_in = -1; return -3;
        }
        got += n;
    }
    gd_fclose((unsigned)h);
    /* ☠️ CHECK THE MAGIC. A stale or truncated card file would otherwise be
       read as offsets and render as garbage geometry, which looks like an
       engine bug and is not one. */
    if (g_lev_arena[0] != 'O' || g_lev_arena[1] != 'L' ||
        g_lev_arena[2] != 'L' || g_lev_arena[3] != 'V') { g_lev_in = -1; return -4; }
    g_lev_in = set;
    return 0;
}

/* blob i of the loaded set: 0 index 1 geom 2 sect 3 atlas 4 pal 5 lara */
static const uint8_t *lev_blob(int i)
{
    const uint8_t *t = g_lev_arena + 8 + i * 8;
    uint32_t o = ((uint32_t)t[0] << 24) | ((uint32_t)t[1] << 16) |
                 ((uint32_t)t[2] << 8)  | (uint32_t)t[3];
    return g_lev_arena + o;
}
#endif  /* LEVSD */
...
static void ring_render(void *fb, int ringA, int spin)
{
    ...
    gpu_geotex_setclip(0, VIEW_W-1, 0, RENDER_H-1);
...
#define CRUMB(c) ((void)0)
    ...
    { extern int jerry_init(void);
      gpu_geotex_setclip(0, VIEW_W-1, 0, RENDER_H-1);   /* clip = full screen */
...
#ifdef LEVSD
        /* ── OVERRIDE FROM THE CARD ───────────────────────────────────────
           Written as an override of the assignment above rather than as a
           replacement of it, so the two paths cannot drift: everything that is
           NOT card data - S_adj, S_portalv, S_portal_ofs, all from the compiled
           *_spawn.h tables - keeps exactly one definition, and only the six
           blobs that were moved change hands here.
           ☠️ S_lskin is NOT one of them. It is Lara's level-independent
           skeleton, shared by both sets through the gym_lskin alias, so it
           stays resident and keeps the value set above. */
        { int lrc = lev_load(g_useset ? 1 : 0);
          if (lrc == 0) {
              S_index = lev_blob(0); S_geom = lev_blob(1); S_sect = lev_blob(2);
              S_atlas = (uint8_t*)lev_blob(3);
              S_pal   = (const uint16_t*)lev_blob(4);
              S_lara  = lev_blob(5);
          } else {
              /* ☠️ DO NOT FALL THROUGH TO THE STUBS. Under LEVSD the symbols
                 above are `.long 0`, so continuing here would dereference a
                 null index and render garbage or hang - a failure that looks
                 like an engine bug and is not one. Park with the code visible
                 in the border instead: -1 no file/card, -2 bad size, -3 read
                 failed, -4 not a level image (stale or truncated card file). */
              *(volatile uint16_t *)0xF00058u =
                  (uint16_t)(0xF000u | ((unsigned)(-lrc) & 0x0Fu) << 4);
              for (;;) ;
          }
        }
#endif
            ...
            { int a, b, c2, d2;
              ...
              for (d2=1; d2<=HOPDEPTH; d2++)
                for (i=0;i<roomCount;i++) {
                  ...
                  for (a=0;a<MRT_ADJ_MAX && S_adj[i][a]!=255;a++) {
                    ...
                    if (!r1) continue;
                    if (r1==2) { a0=0;a1=(VIEW_W-1);b0=0;b1=VIEW_H-1; }
                    ...
                    if (ux0==0 && ux1==(VIEW_W-1) && uy0==0 && uy1==VIEW_H-1) break;
                  }
              ...
              for (i=0;i<roomCount;i++)
                  if (rdepth[i]<=HOPDEPTH && !prv[i]) {
                      prv[i]=1; prx0[i]=0; prx1[i]=(VIEW_W-1);
...
#define SLIVERH 6
...
#endif
                    { int cx0=0, cx1=(VIEW_W-1), cy0=0, cy1=VIEW_H-1;
#ifndef NOPCLIP
                ...
                { int e, nd = 0;
                  ENT_SCAN(door, 8) {
                      ...
                      nd++;
                      displist[1+ndrawn*4+1] = (VIEW_W-1);
                ...
                if (ndrawn) {
                    ...
                    { uint32_t dn = displist[0];
                      ...
                      displist[1+dn*4+1] = (0u<<16) | (VIEW_W-1);
...
#endif
                    gpu_geotex_setclip(0, VIEW_W-1, 0, VIEW_H-1);
